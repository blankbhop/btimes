...
{
    name = "[BTimes 1.9.0] MM Ranks",
    author = "Black_Jesus",
    description = "version 1.9.0",
    url = "http://steamcommunity.com/id/marmon228/"
}
...
ArrayList g_Config = null;
int g_iEloRank[MAXPLAYERS + 1] =  { 0, ... };
 
public OnPluginStart()
{
    char[] szPath = new char[PLATFORM_MAX_PATH];
    BuildPath(Path_SM, szPath, PLATFORM_MAX_PATH, "configs/timer/elo.cfg");
    if(!FileExists(szPath))
        SetFailState("Config file '%s' is not exists", szPath);
   
    KeyValues kv = new KeyValues("Elo");
    if(!kv.ImportFromFile(szPath))
        SetFailState("Error parsing config file '%s'", szPath);
   
    g_Config = new ArrayList(2);
   
    if(kv.GotoFirstSubKey())
    {
        int temp[2];
        char[] szBuffer = new char[16];
       
        do
        {
            kv.GetSectionName(szBuffer, 16);
            temp[0] = kv.GetNum("points");
            temp[1] = StringToInt(szBuffer);
            g_Config.PushArray(temp);
        }
    ...
    // Commands
    RegAdminCmd("sm_ccname", SM_ColoredName, ADMFLAG_ROOT, "Change colored name.");
    RegAdminCmd("sm_ccmsg", SM_ColoredMsg, ADMFLAG_ROOT, "Change the color of your messages.");
    RegAdminCmd("sm_cchelp", SM_Colorhelp, ADMFLAG_ROOT, "For help on creating a custom name tag with colors and a color message.");
 
    RegAdminCmd("sm_rankings", SM_Rankings, ADMFLAG_ROOT, "Shows the chat ranking tags and the ranks required to get them.");
    RegAdminCmd("sm_ranks", SM_Rankings, ADMFLAG_ROOT, "Shows the chat ranking tags and the ranks required to get them.");
    RegAdminCmd("sm_chatranks", SM_Rankings, ADMFLAG_ROOT, "Shows the chat ranking tags and the ranks required to get them.");
    ...
    RegAdminCmd("sm_cclist", SM_CCList, ADMFLAG_ROOT, "Shows a list of players with custom chat privileges.");
    ...
    RegAdminCmd("sm_showcycle", SM_ShowCycle, ADMFLAG_ROOT);
...
{
    RequestFrame(UpdateElo, client);
}
 
int GetEloRank(int points)
{
    int elo = 1;
    int iSize = g_Config.Length;
    if(iSize > 0)
    {
        for (int i = 0; i < iSize; i++)
        {
            if(points < g_Config.Get(i))
            {
                elo = g_Config.Get(i > 0 ? i-1 : i, 1);
                break;
            }
            else
            {
...
public OnStylesLoaded()
{
    ...
    RegConsoleCmdPerStyle("ранг", SM_Rank, "Show your rank for {Type} timer on {Style} style.");
    ...
    RegConsoleCmdPerStyle("топ", SM_Top, "Show list of top players for {Type} timer on {Style} style.");
...
FormatTag(client, String:buffer[], maxlength)
{
    ...
    ReplaceString(buffer, maxlength, "{lightolive}", "\x09", true);
    ReplaceString(buffer, maxlength, "{olive}", "\x10", true);
...
public Action:SM_RecalcPts(client, args)
{
    ...
    SetMenuTitle(menu, "Пересчет баллов занимает некоторое время..\nВы уверены, что хотите это сделать?");
   
    AddMenuItem(menu, "y", "Да");
    AddMenuItem(menu, "n", "Нет");
...
RecalcPoints(client)
{
    CGOPrintToChatAll("[{GREEN}Timer{DEFAULT}] Пересчет рангов см. В консоли прогресса..");
    ...
    if(hndl != INVALID_HANDLE)
    {
        ...
        for(new client = 1; client <= MaxClients; client++)
        {
            if(IsClientInGame(client))
            {
                ...
                {
                    PrintToConsole(client, "[%.1f%%] %s's Точки удалены.",
    ...
    if(GetTypeStyleFromCommand("mapsdone", Type, Style))
    {
        if(!IsSpamming(client))
        {
            ...
            if(args == 0)
            {
                ...
                {
                    CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT] Вы еще не авторизованы таймером.");
                }
            }
            else
            {
                ...
                if(target != -1)
                {
                    ...
                    {
                        CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT] {LIGHTOLIVE}%N{DEFAULT} Еще не авторизован таймером.",
                            target);
                    }
                }
            }
...
OpenStatsMenu(client, PlayerID, Type, Style)
{
    ...
    if(Rank != -1)
    {
        ...
        SetMenuTitle(menu, "Статистика для %s (%s)\n--------------------------------\n", sName, sAuth);
    ...
    else
    {
        ...
        {
            CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] Статистика еще не загружена.");
        }
        else
        {
            ...
            for(new lType; lType < MAX_TYPES; lType++)
            {
                ...
                for(new lStyle; lStyle < MAX_STYLES; lStyle++)
                {
                    if(Style_IsEnabled(lStyle) && Style_IsTypeAllowed(lStyle, lType))
                    {
                        ...
                        {
                            CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] Не удалось найти статистику [{LIGHTOLIVE}%s{DEFAULT}] - [{LIGHTOLIVE}%s{DEFAULT}], Показ статистику для [{LIGHTOLIVE}%s{DEFAULT}] - [{LIGHTOLIVE}%s{DEFAULT}]",
                                sType,
                                sStyle,
                                slType,
                                slStyle);
            ...
            CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] Вы выбрали игрока без рейтинга.");
        }
    }
}
...
public Action:SM_TopWorldRecord(client, args)
{
    ...
    if(GetTypeStyleFromCommand("topwr", Type, Style))
    {
        ...
        if(iSize > 0)
        {
            ...
            SetMenuTitle(menu, "Рекорд сервера [%s] - [%s]", sType, sStyle);
        ...
        {
            CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] [{LIGHTOLIVE}%s{DEFAULT}] - [{LIGHTOLIVE}%s{DEFAULT}] Рекордов сервера на карте нет.",
                sType,
                sStyle);
        }
    }
    ...
    if(!IsSpamming(client))
    {
        ...
        if(g_ClientUseCustom[client] & CC_HASCC)
        {
            ...
            if(args == 0)
            {
                ...
                {
                    PrintColorText(client, "%s%sЦветное имя включено.",
                        g_msg_start,
                        g_msg_textcol);
                }
                else
                {
                    PrintColorText(client, "%s%sЦветное имя отключено.",
                        g_msg_start,
                        g_msg_textcol);
                }
            ...
            else
            {
                ...
                PrintColorText(client, "%s%sЦветное имя установлено на %s%s",
                    g_msg_start,
                    g_msg_textcol,
                    g_msg_varcol,
                    sArg);
        ...
        if(g_ClientUseCustom[client] & CC_HASCC)
        {
            ...
            if(args == 0)
            {
                ...
                if(g_ClientUseCustom[client] & CC_MSGCOL)
                    PrintColorText(client, "%s%sЦветное сообщение включено.",
                        g_msg_start,
                        g_msg_textcol);
                else
                    PrintColorText(client, "%s%sЦветное сообщение отключено.",
                        g_msg_start,
                        g_msg_textcol);
            }
            else
            {
                ...
                PrintColorText(client, "%s%sЦветное сообщение установлено на %s%s",
                    g_msg_start,
                    g_msg_textcol,
                    g_msg_varcol,
                    sArg);
...
public Action:SM_Colorhelp(client, args)
{
    if(GetCmdReplySource() == SM_REPLY_TO_CHAT)
    {
        PrintColorText(client, "%s%sОбратитесь к консоли за помощью в пользовательском чате цветов.",
            g_msg_start,
            g_msg_textcol);
    }
    ...
    PrintToConsole(client, " {rand} will be replaced with RANDOM color.");
...
public Action:SM_ReloadRanks(client, args)
{
    ...
    CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] Рейтинги чата перезагружены.");
...
public EnableCC_Callback1(Handle:owner, Handle:hndl, String:error[], any:pack)
{
    if(hndl != INVALID_HANDLE)
    {
        ...
        if(SQL_GetRowCount(hndl) > 0)
        {
            ...
            if(!(ccuse & CC_HASCC))
            {
                CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] Игроку с именем '[LIGHTOLIVE}%s{DEFAULT}' <[LIGHTOLIVE}%s{DEFAULT}> Будут предоставлены пользовательские привилегии чата.",
                    sName,
                    sAuth);
            ...
            {
                CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] Игрок с данным идентификатором  SteamID '[LIGHTOLIVE}%s{DEFAULT}' (name '[LIGHTOLIVE}%s{DEFAULT}') Уже имеет пользовательские привилегии чата.",
                    sAuth,
                    sName);
            }
        }
        ...
        {
            CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] Ни один игрок в базе данных не найден '[LIGHTOLIVE}%s{DEFAULT}' как их SteamID.",
                sAuth);
        }
    }
    else
...
EnableCustomChat(const String:sAuth[])
{
    ...
    for(new client = 1; client <= MaxClients; client++)
    {
        if(IsClientInGame(client))
        {
            ...
            if(StrEqual(sAuth, sAuth2))
            {
                ...
                PrintColorText(client, "%s%sВам предоставлены пользовательские привилегии чата. Наберите sm_cchelp или попросите о помощи, чтобы узнать, как его использовать.",
                    g_msg_start,
                    g_msg_textcol);
    ...
    FormatEx(query, sizeof(query), "UPDATE players SET ccuse=%d, ccname='{rand}{name}', ccmsgcol='{lime}' WHERE SteamID='%s'",
...
public Action:SM_DisableCC(client, args)
{
    ...
    {
        PrintColorText(client, "%s%ssm_disablecc пример: \"sm_disablecc STEAM_1:1:12345\"",
            g_msg_start,
            g_msg_textcol);
    }
...
public DisableCC_Callback1(Handle:owner, Handle:hndl, String:error[], any:pack)
{
    if(hndl != INVALID_HANDLE)
    {
        ...
        if(SQL_GetRowCount(hndl) > 0)
        {
            ...
            if(ccuse & CC_HASCC)
            {
                CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] Игрок с именем '{LIGHTOLIVE}%s{DEFAULT}' <{LIGHTOLIVE}%s{DEFAULT}> Удалил пользовательские привилегии чата.",
                    sName,
                    sAuth);
            ...
            {
                CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] Игрок с данным SteamID '{LIGHTOLIVE}%s{DEFAULT}' (Имя '{LIGHTOLIVE}%s{DEFAULT}') Пользователь не имеет прав на чат для удаления.",
                    sAuth,
                    sName);
            }
        }
    ...
    for(new client = 1; client <= MaxClients; client++)
    {
        if(IsClientInGame(client))
        {
            ...
            if(StrEqual(sAuth, sAuth2))
            {
                ...
                CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] Вы потеряли свои пользовательские привилегии чата.");
            }
        }
    }
}
...
public CCList_Callback(Handle:owner, Handle:hndl, String:error[], any:client)
{
    if(hndl != INVALID_HANDLE)
    {
        ...
        SetMenuTitle(menu, "Игроки с пользовательскими правами чата");
...
public Menu_CCList(Handle:menu, MenuAction:action, param1, param2)
{
    if (action == MenuAction_Select)
    {
        ...
        PrintColorText(param1, " %sИмя: %s%s", g_msg_textcol, g_msg_varcol, expInfo[1]);
        PrintColorText(param1, " %sCCИмя: %s%s", g_msg_textcol, g_msg_varcol, expInfo[2]);
        PrintColorText(param1, " %sCCообщение: %s%s", g_msg_textcol, g_msg_varcol, expInfo[3]);
...
DB_ShowRank(client, target, Type, Style)
{
    ...
    {
        CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] {LIGHTOLIVE}%N{DEFAULT} Занимает {LIGHTOLIVE}%d{DEFAULT} Из {LIGHTOLIVE}%d{DEFAULT} Игроков с {LIGHTOLIVE}%.1f{DEFAULT} очками.",
            target,
    ...
    {
        CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] {LIGHTOLIVE}%N{DEFAULT} Пока не оценен.",
            target);
    }
}
...
public DB_ShowRank_Callback(Handle:owner, Handle:hndl, String:error[], any:data)
{
    if(hndl != INVALID_HANDLE)
    {
        ...
        if(SQL_FetchInt(hndl, 0) != 0)
        {
            ...
            CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] {LIGHTOLIVE}%s{DEFAULT} Занимает {LIGHTOLIVE}%d{DEFAULT} Из {LIGHTOLIVE}%d{DEFAULT} Игроков с {LIGHTOLIVE}%.1f{DEFAULT} очками.",
                sTarget,
                Rank,
                Total,
                Points);
        ...
        {
            CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] {LIGHTOLIVE}%s{DEFAULT} Пока не оценен.",
                sTarget);
        }
    }
    else
...
public DB_ShowTopAll_Callback(Handle:owner, Handle:hndl, String:error[], any:data)
{
    if(hndl != INVALID_HANDLE)
    {
        ...
        SetMenuTitle(menu, "ТОП 100 игроков\n------------------------------------");
...
{    
    ...
    if(iSize > 0)
    {
        ...
        SetMenuTitle(menu, "ТОП 100 игроков %s - %s\n--------------------------------------", sType, sStyle);
        ...
        for(new idx; idx < iSize && idx < 100; idx++)
        {
            ...
            Format(sDisplay, sizeof(sDisplay), "#%d: %s (%d Очков.)", idx + 1, sDisplay, RoundToNearest(GetArrayCell(g_hRanksPoints[Type][Style], idx)));
    ...
    {
        CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] {LIGHTOLIVE}%s{DEFAULT} -{LIGHTOLIVE} %s {DELAULT}Пока еще нет ранговых игроков.",
            sType,
            sStyle);
    }
}
...
DB_ShowMapsleft(client, target, Type, Style)
{
    ...
    else
    {
        if(client == target)
        {
            CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] Ваш SteamID не авторизован. Серверы могут быть недоступны. Если нет, попробуйте переподключиться.");
        }
        else
        {
            ...
            CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] {LIGHTOLIVE}%s's {DEFAULT}SteamID Не авторизован. Серверы могут быть недоступны.",
                name);
        }
    }
}
    ...
    if(hndl != INVALID_HANDLE)
    {
        ...
        if(client != 0)
        {
            ...
            if (rows > 0)
            {
                ...
                {
                    Format(title, sizeof(title), "%d %sКарты завершены%s",
                        count,
                        sType,
                        sStyle);
                }
                ...
                {
                    Format(title, sizeof(title), "%d %sКарты завершены%s Для игрока %s",
                        count,
                        sType,
                        sStyle,
                        sTarget);
            ...
            else
            {
                if(clientUserId == targetUserId)
                {
                    CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] {LIGHTOLIVE}%s{DEFAULT} У вас нет карт, которые нужно бить{LIGHTOLIVE}%s{DEFAULT}.",
                        sType,
                        sStyle);
                }
                else
                {
                    CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] {LIGHTOLIVE}%s%s{DEFAULT} Не осталось карт{LIGHTOLIVE}%s{DEFAULT}.",
                        sType,
                        sTarget,
                        sStyle);
                }
...
DB_ShowMapsdone(client, PlayerID, Type, Style)
{
    ...
    if(hCell != INVALID_HANDLE)
    {
        ...
        SetMenuTitle(menu, "Карты, сделанные для %s [%s] - [%s]\n \nЗавершено %d / %d\n-----------------------------------", sName, sType, sStyle, iSize, GetTotalZonesAllMaps((Type == TIMER_MAIN)?MAIN_START:BONUS_START));
       
        DisplayMenu(menu, client, MENU_TIME_FOREVER);
    }
    else
    {
        CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] {LIGHTOLIVE}%s {DEFAULT}Не имеет карт.",
            sName);
    }
}
...
public Menu_ShowMapsdone_Callback(Handle:owner, Handle:hndl, String:error[], any:data)
{
    if(hndl != INVALID_HANDLE)
    {
        ...
        if(rows != 0)
        {
            ...
            if(client == target)
            {
                SetMenuTitle(menu, "%s%d Карты, пройденные%s",
                    sType,
                    mapsdone,
                    sStyle);
            }
            else
            {
                ...
                SetMenuTitle(menu, "%s%d Карты, пройденные%s%s",
                    sType,
                    mapsdone,
                    sTargetName,
                    sStyle);
        ...
        else
        {
            if(client == target)
            {
                CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] {LIGHTOLIVE}%s {DEFAULT}Вы еще не закончили карты{LIGHTOLIVE}%s{DEFAULT}.",
                    sType,
                    sStyle);
            }
            else
            {
                ...
                CGOPrintToChat(client, "[{GREEN}Timer{DEFAULT}] {LIGHTOLIVE}%s%s {DEFAULT}Не имеет готовых карт{LIGHTOLIVE}%s{DEFAULT}.",
                    sType,
                    targetname,
                    sStyle);
            }
        ...
        if(recalc == true)
        {
            ...
            for(new client = 1; client <= MaxClients; client++)
            {
                if(IsClientInGame(client))
                {
                    if(!IsFakeClient(client))
                    {
                        PrintToConsole(client, "[%.1f%%] %s %s%s Законченный пересчет.",
                            float(g_RecalcProgress)/float(g_RecalcTotal) * 100.0,
                            sMapName,
                            sType[Type],
                            sStyle[Style]);
...
public void UpdateElo(any client)
{
    g_iEloRank[client] = GetEloRank((g_Rank[client][TIMER_MAIN][0] != 0) ? RoundToNearest(GetArrayCell(g_hRanksPoints[TIMER_MAIN][0], g_Rank[client][TIMER_MAIN][0] - 1)) : 0);
}
...
public PlayerManager_OnThinkPost(entity)
{
    ...
    SetEntDataArray(entity, iRankOffset, g_iEloRank, MAXPLAYERS+1, _, true);
}
